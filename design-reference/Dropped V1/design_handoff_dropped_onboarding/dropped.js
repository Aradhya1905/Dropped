/* DROPPED — onboarding scaffolding & ambient setup */
(function () {
  // ---- living map texture (shared SVG) ----
  const MAP_SVG = `
  ...
  document.querySelectorAll('[data-map]').forEach(el => { el.innerHTML = MAP_SVG; });
  document.querySelectorAll('[data-status]').forEach(el => { el.innerHTML = STATUS; });

  // ---- deliberate street map for the "in range" sequence ----
  const ROUTEMAP = `
  ...
  document.querySelectorAll('[data-routemap]').forEach(el => { el.innerHTML = ROUTEMAP; });
  document.querySelectorAll('[data-routeline]').forEach(el => { el.innerHTML = ROUTELINE; });

  // ---- split wordmark into animated characters ----
  ...
  // ---- fit the phone rails to the viewport ----
  const rails = [...document.querySelectorAll('.rail')];
  function fit() {
    rails.forEach(rail => {
      rail.style.transform = 'none';
      const natural = rail.scrollWidth;
      ...
      wrap.style.height = (rail.getBoundingClientRect().height) + 'px';
    });
  }
  window.addEventListener('resize', fit);
  // re-fit once fonts have settled
  fit();
  if (document.fonts && document.fonts.ready) document.fonts.ready.then(fit);
  setTimeout(fit, 400);
  window.addEventListener('load', fit);

  // expose for tweaks layer
  window.__droppedFit = fit;

  // ---- one-time entrance reveal (steady state stays visible) ----
  function playIntro() {
    document.body.classList.add('intro-active');
    setTimeout(() => document.body.classList.remove('intro-active'), 2600);
  }
